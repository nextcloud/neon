...
void main() {
  presets('spreed', 'spreed', (tester) {
    Future<spreed.Room> createTestRoom() async =>
        (await tester.client.spreed.room.createRoom(
    ...
    group('Helpers', () {
      ...
      test('Participant permissions', () async {
        ...
        expect(
          {spreed.ParticipantPermission.custom, spreed.ParticipantPermission.canSendMessageAndShareAndReact}.binary,
          129,
        );
      });
    });
    ...
    group('Room', () {
      ...
      test('Message expiration', () async {
        ...
        await tester.client.spreed.room.setMessageExpiration(
          token: room.token,
          $body: spreed.RoomSetMessageExpirationRequestApplicationJson((b) => b.seconds = expiration),
        );

        final clearHistoryResponse = await tester.client.spreed.chat.clearHistory(token: room.token);
        ...
        final getRoomResponse = await tester.client.spreed.room.getSingleRoom(token: room.token);
    ...
    group('Avatar', () {
      test('Set emoji avatar', () async {
        ...
        final response = await tester.client.spreed.avatar.emojiAvatar(
          token: room.token,
          $body: spreed.AvatarEmojiAvatarRequestApplicationJson((b) => b..emoji = '😀'),
        );
        expect(response.statusCode, 200);
      ...
      test('Get avatar', () async {
        ...
        final response = await tester.client.spreed.avatar.getAvatar(token: room.token);
        expect(response.statusCode, 200);
      ...
      test('Get avatar dark', () async {
        ...
        final response = await tester.client.spreed.avatar.getAvatarDark(token: room.token);
        expect(response.statusCode, 200);
        expect(response.body, isNotEmpty);
      });
      ...
      test('Delete avatar', () async {
        ...
        final response = await tester.client.spreed.avatar.deleteAvatar(token: room.token);
        expect(response.statusCode, 200);
    ...
    group('Chat', () {
      test('Send message', () async {
        ...
        final response = await tester.client.spreed.chat.sendMessage(
          token: room.token,
          $body: spreed.ChatSendMessageRequestApplicationJson((b) => b..message = 'bla'),
        );
        expect(response.body.ocs.data!.id, isPositive);
      ...
      test('Edit message', () async {
        ...
        final response = await tester.client.spreed.chat.editMessage(
          ...
          $body: spreed.ChatEditMessageRequestApplicationJson((b) => b..message = '123'),
        );
        expect(response.body.ocs.data.id, isPositive);
        expect(response.body.ocs.data.actorType, spreed.ActorTypes.users);
        expect(response.body.ocs.data.actorId, 'user1');
        ...
        expect(response.body.ocs.data.parent!.chatMessage!.lastEditTimestamp, closeTo(startTime.secondsSinceEpoch, 10));
      ...
      group('Get messages', () {
        test('Directly', () async {
          ...
          expect(response.body.ocs.data[0].parent!.chatMessage!.timestamp, closeTo(startTime.secondsSinceEpoch, 10));
        ...
        test('Polling', () async {
          ...
          unawaited(
            Future<void>.delayed(const Duration(seconds: 1)).then((_) async {
              await tester.client.spreed.chat.sendMessage(
                token: room.token,
                $body: spreed.ChatSendMessageRequestApplicationJson((b) => b..message = '123'),
              );
            }),
          );
      ...
      test('Mention suggestions', () async {
        ...
        await tester.client.spreed.room.addParticipantToRoom(
          token: room.token,
          $body: spreed.RoomAddParticipantToRoomRequestApplicationJson((b) => b..newParticipant = 'user2'),
        );

        final response = await tester.client.spreed.chat.mentions(token: room.token, search: 'user');
    ...
    group('Signaling', () {
      test('Get settings', () async {
        ...
        final response = await tester.client.spreed.internalSignaling.signalingGetSettings(token: room.token);
        ...
        expect(response.body.ocs.data.helloAuthParams.$20.token.split('').where((x) => x == '.'), hasLength(2));
        ...
        expect(response.body.ocs.data.turnservers[0].urls[0], 'turn:staticauth.openrelay.metered.ca:443?transport=udp');
        expect(response.body.ocs.data.turnservers[0].urls[1], 'turn:staticauth.openrelay.metered.ca:443?transport=tcp');
        expect(
      ...
      test('Send and receive messages', () async {
        ...
        final client2 = await tester.createClient(username: 'user2');
                  ...
                  ..messages = json.encode([
                    {
                      ...
                      'fn': json.encode({'to': room2.sessionId}),
                    },
                  ]),
          ),
        );
    ...
    group('Reaction', () {
      test('Add and remove', () async {
        ...
        final addResponse = await tester.client.spreed.reaction.react(
          ...
          $body: spreed.ReactionReactRequestApplicationJson((b) => b..reaction = '😀'),
        );
