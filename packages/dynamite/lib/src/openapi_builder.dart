...
class OpenAPIBuilder implements Builder {
  OpenAPIBuilder(BuilderOptions options) : buildConfig = DynamiteConfig.fromJson(options.config);
    ...
    try {
      ...
      final output = Library((b) {
        ...
        b
          ...
          ..docs.addAll(escapeDescription(state.spec.info.formattedDescription()))
          ..directives.addAll([
      ...
      if (coverageIgnores != null) {
        for (final ignore in coverageIgnores) {
          ...
          if (outputString == before) {
            log.warning('Coverage ignore pattern $ignore did not match anything');
          }
        }
      }
      ...
      unawaited(buildStep.writeAsString(state.outputId, formatter.format(outputString)));
    } catch (error, stackTrace) {
      log.severe('Issue generating the library for $state.inputId', error, stackTrace);
