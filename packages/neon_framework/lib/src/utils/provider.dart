...
class NeonProvider<T extends Disposable> extends SingleChildStatelessWidget {
  ...
  const NeonProvider({required Create<T> create, super.key, this.child, this.lazy = true})
    : _create = create,
      _value = null,
      super(child: child);
  ...
  const NeonProvider.value({required T value, super.key, this.child})
    : _value = value,
      _create = null,
      lazy = true,
      super(child: child);
  ...
  Widget buildWithChild(BuildContext context, Widget? child) {
    assert(child != null, '$runtimeType used outside of MultiBlocProvider must specify a child');
    final value = _value;
    return value != null
        ? InheritedProvider<T>.value(value: value, lazy: lazy, child: child)
        : InheritedProvider<T>(create: _create, dispose: (_, value) => value.dispose(), lazy: lazy, child: child);
  }
}
