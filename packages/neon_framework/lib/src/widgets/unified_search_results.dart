...
class NeonUnifiedSearchResults extends StatelessWidget {
  const NeonUnifiedSearchResults({required this.onSelected, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder:
          (context, isExtendedSearchSnapshot) => ResultBuilder.behaviorSubject(
            subject: bloc.providers,
            builder:
                (context, providers) => StreamBuilder(
                  stream: bloc.results,
                  builder: (context, resultsSnapshot) {
                    ...
                    return NeonListView(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _buildResult(context: context, provider: provider, result: result);
                      },
                    );
                  },
                ),
  ...
  Widget _buildResult({
    ...
    final children = <Widget>[
      if (!showCupertino) DefaultTextStyle(style: Theme.of(context).textTheme.headlineSmall!, child: title),
      if (result.hasError) NeonError(result.error, onRetry: bloc.refresh),
    ...
    if (isCupertino(context)) {
      return CupertinoListSection.insetGrouped(header: title, children: children);
    } else {
      return Card(
        child: Container(
  ...
  Widget _buildNoResultsTile(BuildContext context) {
    return AdaptiveListTile(
      leading: Icon(AdaptiveIcons.close, size: largeIconSize),
  ...
  Widget? _buildFallbackIcon(BuildContext context, Account account, core.UnifiedSearchResultEntry entry) {
    if (entry.icon.startsWith('/')) {
      return NeonUriImage(size: const Size.square(largeIconSize), uri: Uri.parse(entry.icon), account: account);
    }
