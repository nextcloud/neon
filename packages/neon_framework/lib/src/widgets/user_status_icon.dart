...
class NeonUserStatusIcon extends StatelessWidget {
  ...
  const NeonUserStatusIcon({required this.type, this.size, super.key});
  ...
  Widget build(BuildContext context) {
    return switch (type) {
      user_status.$Type.online => Icon(size: size, Icons.circle, color: NcColors.success),
      user_status.$Type.dnd => Icon(size: size, MdiIcons.minusCircle, color: NcColors.error),
      user_status.$Type.away => Transform.rotate(
        angle: _minus45Rad,
        child: Icon(size: size, MdiIcons.moonWaningCrescent, color: NcColors.warning),
      ),
