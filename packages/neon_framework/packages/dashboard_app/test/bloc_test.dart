          ...
          'ocs': {
            ...
            'data': {
              for (final key in request.url.queryParametersAll['widgets[]']!)
                key: [
                  {'subtitle': '', 'title': key, 'link': '', 'iconUrl': '', 'overlayIconUrl': '', 'sinceId': ''},
                ],
              'tooMany1': [
                for (var i = 0; i < 8; i++)
                  {'subtitle': '', 'title': '$i', 'link': '', 'iconUrl': '', 'overlayIconUrl': '', 'sinceId': ''},
              ],
            },
          },
        }),
...
void main() {
  ...
  setUp(() {
    ...
    bloc = DashboardBloc(account: account);
  });
  ...
  test('refresh', () async {
    final widgets = BuiltList<String>(['v1', 'v2', 'v1v2']);
    expect(
      bloc.widgets.transformResult((e) => BuiltList<String>(e.map((w) => w.id))),
      emitsInOrder([Result<BuiltList<String>>.loading(), Result.success(widgets)]),
    );
