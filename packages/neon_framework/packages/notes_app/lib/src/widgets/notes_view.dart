...
import 'package:notes_app/src/blocs/note.dart';
import 'package:notes_app/src/blocs/notes.dart';
import 'package:notes_app/src/options.dart';
import 'package:notes_app/src/pages/note.dart';
import 'package:notes_app/src/sort/notes.dart';
import 'package:notes_app/src/utils/category_color.dart';
import 'package:timezone/timezone.dart' as tz;

class NotesView extends StatelessWidget {
  const NotesView({required this.bloc, this.category, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return ResultBuilder.behaviorSubject(
      subject: bloc.notes,
      builder:
          (context, notesList) => SortBoxBuilder(
            sortBox: notesSortBox,
            presort: const {(property: NotesSortProperty.favorite, order: SortBoxOrder.ascending)},
  ...
  Widget _buildNote(BuildContext context, notes.Note note) => Dismissible(
    key: Key(note.id.toString()),
    direction: DismissDirection.startToEnd,
    behavior: HitTestBehavior.translucent,
    background: Container(color: Colors.red),
    confirmDismiss:
        (direction) async => showConfirmationDialog(
          context: context,
    ...
    child: ListTile(
      ...
      subtitle: Row(
        children: [
          RelativeTime(date: DateTimeUtils.fromSecondsSinceEpoch(tz.UTC, note.modified)),
          if (note.category.isNotEmpty) ...[
            const SizedBox(width: 8),
            Icon(AdaptiveIcons.tag, size: smallIconSize, color: NotesCategoryColor.compute(note.category)),
      ...
      trailing: IconButton(
        onPressed: () {
          bloc.updateNote(note.id, note.etag, favorite: !note.favorite);
        },
      ...
      onTap: () async {
        await Navigator.of(context).push(
          MaterialPageRoute<void>(
            builder:
                (context) => NotesNotePage(
                  bloc: NotesNoteBloc(notesBloc: bloc, account: NeonProvider.of<Account>(context), note: note),
                  notesBloc: bloc,
                ),
          ),
        );
