...
class NeonRichObjectMention extends StatelessWidget {
  ...
  const NeonRichObjectMention({required this.parameter, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    switch (parameter.type) {
      case core.RichObjectParameter_Type.user:
        ...
        child = NeonUserAvatar(username: parameter.id, account: NeonProvider.of<Account>(context));
      case core.RichObjectParameter_Type.call:
        highlight = true;
        child = CircleAvatar(
          child: ClipOval(
            child: NeonUriImage(uri: Uri.parse(parameter.iconUrl!), account: NeonProvider.of<Account>(context)),
          ),
        );
      case core.RichObjectParameter_Type.guest:
        // TODO: Add highlighting when the mention is about the current guest user.
        highlight = false;
        child = CircleAvatar(child: Icon(AdaptiveIcons.person));
      case core.RichObjectParameter_Type.userGroup:
        ...
        child = CircleAvatar(child: Icon(AdaptiveIcons.group));
      default:
    ...
    return Chip(
      ...
      labelPadding: const EdgeInsetsDirectional.only(end: 4, start: -4, top: -2, bottom: -2),
      avatar: Padding(padding: const EdgeInsets.all(6), child: child),
      visualDensity: const VisualDensity(
        horizontal: VisualDensity.minimumDensity,
        vertical: VisualDensity.minimumDensity,
      ),
      label: Text(parameter.name, style: TextStyle(color: foregroundColor)),
