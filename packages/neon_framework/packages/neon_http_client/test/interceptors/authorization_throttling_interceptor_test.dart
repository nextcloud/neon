...
final jsonHeaders = <String, String>{'content-type': 'application/json; charset=utf-8'};
...
void main() {
  ...
  group(AuthorizationThrottlingInterceptor, () {
    test('blocks after unauthorized response', () async {
      ...
      expect(interceptor.shouldInterceptRequest(authorizedRequest), isFalse);

      expect(interceptor.shouldInterceptResponse(authorizedResponse), isTrue);
      await expectLater(
        interceptor.interceptResponse(response: authorizedResponse, url: authorizedRequest.url),
        completion(isA<StreamedResponse>()),
      );

      expect(interceptor.shouldInterceptRequest(authorizedRequest), isTrue);
      await expectLater(
    ...
    test('unblocks after successful poll', () async {
      ...
      final pollRequest = Request('POST', Uri.parse('$baseURL/index.php/login/v2/poll'));
      final pollResponse = StreamedResponse(const Stream.empty(), 200, request: pollRequest);
      ...
      expect(interceptor.shouldInterceptRequest(pollRequest), isFalse);
      expect(interceptor.shouldInterceptResponse(pollResponse), isTrue);
      await expectLater(
    ...
    test('never blocks requests not matching baseURL', () async {
      ...
      expect(interceptor.shouldInterceptRequest(otherServerRequest), isFalse);
      expect(interceptor.shouldInterceptResponse(otherServerResponse), isFalse);

      expect(interceptor.shouldInterceptRequest(correctServerRequest), isFalse);
      expect(interceptor.shouldInterceptResponse(correctServerResponse), isTrue);
      await expectLater(
        interceptor.interceptResponse(response: correctServerResponse, url: correctServerRequest.url),
        completion(isA<StreamedResponse>()),
      );
      expect(interceptor.shouldInterceptRequest(correctServerRequest), isTrue);
      await expectLater(
    ...
    test('never blocks requests without authorization', () async {
      ...
      expect(interceptor.shouldInterceptRequest(unauthorizedRequest), isFalse);
      expect(interceptor.shouldInterceptResponse(unauthorizedResponse), isTrue);
      await expectLater(
