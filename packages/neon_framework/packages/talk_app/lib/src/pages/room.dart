  ...
  Widget build(BuildContext context) {
    return ResultBuilder.behaviorSubject(
      ...
      builder: (context, result) {
        ...
        Widget title = Text(room.displayName, overflow: TextOverflow.ellipsis);

        final statusMessage = room.statusMessage;
        if (statusMessage != null) {
          title = Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [title, Text(statusMessage, style: Theme.of(context).textTheme.labelSmall)],
          );
        }
        ...
        final appBar = AppBar(
          title: Row(
            spacing: 10,
            children: <Widget>[
              TalkRoomAvatar(room: room),
              title,
              NeonError(result.error, onRetry: bloc.refresh, type: NeonErrorType.iconOnly),
              Flexible(child: NeonLinearProgressIndicator(visible: result.isLoading)),
            ],
          ),
        );
        ...
        Widget body = StreamBuilder(
          ...
          builder:
              (context, lastCommonReadSnapshot) => ResultBuilder.behaviorSubject(
                ...
                builder: (context, messagesResult) {
                  ...
                  return NeonListView.custom(
                    ...
                    slivers: [SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 10), sliver: sliver)],
                  );
                },
              ),
        );
