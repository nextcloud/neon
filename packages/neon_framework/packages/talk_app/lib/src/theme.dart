...
class TalkTheme extends ThemeExtension<TalkTheme> {
  ...
  const TalkTheme({this.messageConstraints = const BoxConstraints(maxWidth: 1120)});
  ...
  TalkTheme copyWith({BoxConstraints? messageConstraints}) {
    return TalkTheme(messageConstraints: messageConstraints ?? this.messageConstraints);
  }
  ...
  TalkTheme lerp(covariant TalkTheme? other, double t) {
    ...
    return TalkTheme(messageConstraints: BoxConstraints.lerp(messageConstraints, other.messageConstraints, t)!);
  }
}
