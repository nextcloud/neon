...
class FileListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      title: Text(details.name, overflow: TextOverflow.ellipsis),
      subtitle: Row(
        children: [
          if (details.lastModified != null) RelativeTime(date: details.lastModified!),
          if (details.size != null && details.size! > 0) ...[
            const SizedBox(width: 10),
            Text(filesize(details.size, 1), style: DefaultTextStyle.of(context).style.copyWith(color: Colors.grey)),
          ],
        ],
      ),
      leading: _FileIcon(details: details, bloc: bloc),
      trailing:
          !details.hasTask && browserBloc.mode == FilesBrowserMode.browser
...
class _FileIcon extends StatelessWidget {
  const _FileIcon({required this.details, required this.bloc});
              ...
              ? StreamBuilder(
                ...
                builder:
                    (context, progress) => Column(
                      children: [
                        Icon(switch (details.task!) {
                          FilesUploadTask() => MdiIcons.upload,
                          FilesDownloadTask() => MdiIcons.download,
                        }, color: Theme.of(context).colorScheme.primary),
                        LinearProgressIndicator(value: progress.data),
                      ],
                    ),
              )
              : FilePreview(bloc: bloc, details: details),
    );
      ...
      icon = Stack(
        children: [
          ...
          Align(
            ...
            child: Icon(AdaptiveIcons.star, size: smallIconSize, color: Colors.yellow),
          ),
        ],
      );
    }

    return SizedBox.square(dimension: largeIconSize, child: icon);
  }
}
