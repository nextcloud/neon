...
import 'package:news_app/src/pages/folder.dart';
import 'package:news_app/src/sort/folders.dart';
import 'package:news_app/src/utils/dialog.dart';

class NewsFoldersView extends StatelessWidget {
  const NewsFoldersView({required this.bloc, super.key});
                ...
                (context, feeds) => SortBoxBuilder(
                  ...
                  builder:
                      (context, sorted) => NeonListView(
                        ...
                        itemBuilder: (context, index) => _buildFolder(context, sorted[index]),
                      ),
                ),
          ),
    );
  ...
  Widget _buildFolder(BuildContext context, FolderFeedsWrapper folderFeedsWrapper) {
    ...
    return Dismissible(
      ...
      child: ListTile(
        ...
        leading: SizedBox.square(
          ...
          child: Stack(
            children: [
              Icon(AdaptiveIcons.folder, size: largeIconSize, color: Theme.of(context).colorScheme.primary),
              Center(
                child: Text(feedCount.toString(), style: TextStyle(color: Theme.of(context).colorScheme.onPrimary)),
              ),
            ],
          ),
        ),
        ...
        onTap:
            folder != null
                ? () async {
                  await Navigator.of(
                    context,
                  ).push(MaterialPageRoute<void>(builder: (context) => NewsFolderPage(bloc: bloc, folder: folder)));
                }
                : null,
      ),
    );
...
enum NewsFolderAction { delete, rename }
